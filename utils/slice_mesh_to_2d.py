...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to the root folder containing the volumes",
    )
    parser.add_argument(
        "--output_size", type=int, default=256, help="Desired output size of the slices"
    )
...
def slice_volumes(args):
    ...
    for folder in folders:
        ...
        for i, z_level in enumerate(z_levels):
            ...
            borders = (points[np.abs(points[:, 2] - z_level) < 0.1] * 200)[
                :, :2
            ].astype(np.uint8)
