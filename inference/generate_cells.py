...
def process(
    ...
    with torch.no_grad():
        ...
        input_image = einops.rearrange(
            input_image[None, ...], "b h w c -> b c h w"
        ).clone()
        cond = {
        ...
        un_cond = {
            "c_concat": None if guess_mode else [input_image],
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to the pregenerated conditional masks",
    )
    parser.add_argument(
        "--out_path", type=str, required=True, help="Path to save the outputs"
    )
    parser.add_argument(
        "--prompt", type=str, default="", help="Text prompt for generation"
    )
    parser.add_argument(
        "--n_prompt", type=str, default="", help="Negative text prompt for generation"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--guess_mode", type=bool, default=True, help="Guess mode for generation"
    )
    parser.add_argument(
        "--strength",
        type=float,
        default=2.0,
        help="Control strength for the text prompt",
    )
    parser.add_argument(
        "--scale",
        type=float,
