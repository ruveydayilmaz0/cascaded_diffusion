...
class HeLaDataset(Dataset):
    ...
    def prepare_img_and_mask(self, idx):
        ...
        mask = torch.squeeze(
            F.interpolate(
                mask[None, None, ...],
                [mask.shape[0] * 8, mask.shape[1] * 8],
                mode="nearest",
            )
        )  ##
        # mask = torch.squeeze(F.interpolate(mask[None,None,...], [mask.shape[0]*6, mask.shape[1]*6], mode='nearest'))

        # Make RGB
        # return img[...,None].repeat(2, 2, 3)[:512,:512,:], mask[...,None].repeat(2, 2, 3)[:512,:512,:]
        # to skip the autoencoder, add 4 channels to the input and make it 64x64
        return img[..., None].repeat(1, 1, 4)[16:80, 16:80, :], mask[..., None].repeat(
            1, 1, 3
        )  ##
